...
// Styled component to add the zoom effect on hover
const ZoomCard = styled(Card)({
  transition: "transform 0.3s ease-in-out", // Smooth transition
  "&:hover": {
    transform: "scale(1.05)", // Scale up slightly on hover
    boxShadow: "0 8px 16px rgba(0, 0, 0, 0.2)", // Add a deeper shadow on hover
  },
});
...
const Blog = () => {
  ...
  const handleDelete = async (id) => {
    try {
      await axios.delete(
        `https://fyndah-backend.onrender.com/api/blogs/${id}`,
        {
  ...
  const handleEditSubmit = async () => {
    try {
      const response = await axios.put(
        `https://fyndah-backend.onrender.com/api/blogs/${editBlog._id}`,
        editBlog,
        {
          headers: { Authorization: `Bearer ${localStorage.getItem("token")}` },
        }
